...
function filter_inventory_item(user_data, manifest, /**/ wep_formulas, exotic_armor_perks, wep_perks) {
    ...
    for (let i = 0; i < item_ids.length; i++) {
        ...
        if (item_ids[i][2] == 3) { // if item type is weapon
            new_item_list[unique_id] = make_new_weapon(item, unique_id)
            return
        }
        if (item_ids[i][2] == 2 && item.inventory.tierTypeName == 'Exotic') { // if item type is armor and exotic
            new_item_list[unique_id] = make_new_armor(item, unique_id)
        }
    }
    function make_new_weapon(item) {
        let weapon_data = new Weapon_constructor(item, unique_id)
        return {
            ...
            'item_type': 'weapon',
            'perks': {
                'active_perks': weapon_data.all_perks(user_data, json.inventory_item, unique_id),
                'perks': weapon_data.all_perks(item, unique_id)
            },
...
class Weapon_constructor {
    ...
    all_perks() { // makes array with arrays holding perk id's
        this.item.sockets.socketCategories.find(x => x.socketCategoryHash == 4241085061).socketIndexes.forEach(find_perk_ids) // for each socked index
        ...
        function find_perk_ids(index) {
            ...
            if (random_perks?.plugs[index]) { // check if weapon has random perks
                random_perks.plugs[index].forEach(perk_id => perk_list.push(get_perk_description(perk_id, item, unique_id, index)))
                ...
                if (reusable_plug_set_id) {
                    json.plug_set[reusable_plug_set_id].reusablePlugItems.forEach(perk_id => perk_list.push(get_perk_description(perk_id, item, unique_id, index)))
                } else if (reusable_plug_items) {
                    reusable_plug_items.forEach(perk_id => perk_list.push(get_perk_description(perk_id, item, unique_id, index)))
                }
            }
            perk_list.filter(perk_id => json.inventory_item[perk_id].plug.plugCategoryHash
                .indexOf(x => x == this.perk_types) != -1
            ) // get rid of trash pretending to be perk
    ...
    active_perks() {
        ...
        .filter(active_perk => active_perk.plugHash != undefined && this.perk_types
            .indexOf(inventory_item[active_perk.plugHash].plug.plugCategoryHash) > -1
        )
        .map(active_perk => active_perk.plugHash)
    }
}
function get_perk_description(perk_id, item, unique_id, index) {
    return {
        'id': perk_id,

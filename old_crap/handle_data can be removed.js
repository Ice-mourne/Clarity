...
function sort_data(user_data, manifest, wep_formulas, community_data) {
    ...
    function find_item_ids() {
        ...
        Object.values(user_data.characterInventories.data).forEach(x => all_items = [...all_items, x.items])
        Object.values(user_data.characterEquipment  .data).forEach(x => all_items = [...all_items, x.items])

        let item_ids = []
        for (let i = 0; i < all_items.length; i++) {
            const unique_id = all_items[i].itemInstanceId
            if(!unique_id) continue // check if it has instanced (unique) id
            if(old_data && // first time this this is undefined and everything after can be skipped
                (
                    old_data[unique_id] || // check is it new id
                    JSON.stringify(new_data[unique_id]) == JSON.stringify(old_data[unique_id]) // check if it same as last time
                )
            ) continue

            item_ids.push({
                'unique_id': unique_id,
                'static_id': all_items[i].itemHash
            })
        }
        old_data = user_data.itemComponents.sockets.data // store for next time
        return item_ids
    }
    let item_ids = find_item_ids()
    for (let i = 0; i < item_ids.length; i++) {
        const unique_id = item_ids[i].unique_id
        const item = manifest.inventory_item[item_ids[i].static_id]

        if (item.itemType == 3                                           )
        clarity_inventory[unique_id] = JSON.parse(JSON.stringify(weapon(unique_id, item)))

        if (item.itemType == 2 && item.inventory.tierTypeName == 'Exotic')
        clarity_inventory[unique_id] = JSON.parse(JSON.stringify(armor(unique_id, item)))
    }
    ...
    add_more_info(clarity_inventory, manifest, wep_formulas, community_data)
    console.log(clarity_inventory);

...
function gunsmit_button(){
    ...
    if(document.getElementById('gunsmith_button') == undefined){
        create_element({'ele_type': 'div','id': 'gunsmith_button','class': json_divs.class,'sw_before_last': json_divs.location});
    ...
    if (stat_window.getElementsByClassName(json_divs.buton_text_hide)[0] != undefined){
        document.getElementById('gunsmith_button_text').style.cssText = 'display: none;';
    };
    stat_window.getElementsByClassName(json_divs.click_event)[0].addEventListener('click', e => {
        if (stat_window.getElementsByClassName(json_divs.buton_text_hide)[0] == undefined){
            document.getElementById('gunsmith_button_text').style.cssText = 'display: none;';
        } else {
...
function open_gunsmith(){
    let json_divs = JSON.parse(localStorage.getItem('clarity_dim_div_locations')).d2_gunsmith;
    let wep_ids = document.getElementById(json_divs.stat_window).nextSibling.querySelector(json_divs.link).href.replace('https://destinytracker.com/destiny-2/db/items/', '').replace('?perks=', ',').replace('4248210736', '0').split(',');
    ...
    document.getElementById('gunsmith_close').addEventListener('click', e => {
        document.getElementById('gunsmith').remove();
    });
    allow_drag(); 
};
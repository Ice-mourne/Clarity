function rework_weapon_perks() {
    ...
    function click_new_perk(event) { // closes open perk in DIM and removes text from new description then clicked on new description
        pause_event = false
    ...
    function click_DIM_perk(event){
        ...
        setTimeout(() => {
            if (pause_event) {
                ...
                let final_description = add_meaningful_numbers(perk_description)
                ...
                if(target.getElementsByClassName('plug-stats')[0]){
                    ...
                    for (let i = 0; i < stats.childNodes.length; i++) {
                        ...
                        if ((element == 'Range' || element == 'Zoom') && check == 0) {
                            ...
                            if (final_range != 'NaNm') { // checks if final_range is valid value
    ...
    function add_meaningful_numbers(data){
        ...
        if (data && data.range_stat) {
            ...
            let meters =  sessionStorage.getItem('weapon_range') * 1
            ...
            for (let i = 0; i < data.range_stat.length; i++) {
                ...
                let final_range = (range_metters - meters > 0) ? `+${(range_metters - meters).toFixed(2)}` : (range_metters - meters).toFixed(2)
